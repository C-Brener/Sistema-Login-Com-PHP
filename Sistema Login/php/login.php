<?php
    session_start(); // serve para indicar ao php que desejamos trabalhar com session e que estamos inciando-a
    include("conexao.php"); // Incluindo o arquivo de conexão do php
    ...
    if(empty($_POST['crn']) && empty($_POST['senha'])){ // fazendo validação para que caso os campos sejam vazios não possam ser acesssado o login.php
        header('Location: ../index.php'); // Indicando para onde será redirecionado caso o primeiro parametro retorne true
        exit();
    }
    ...
    $linha = mysqli_num_rows($resultado); // verificando a linha puxada pela variavel resultado, caso seja 0 significa que nao existe no db
    if($linha == 1){
